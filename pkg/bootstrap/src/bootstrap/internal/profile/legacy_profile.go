...
// This is a bootstrap copy of /Users/bhavyabhatt/Desktop/bhavya/projects/go-plus/src/internal/profile/legacy_profile.go

//line /Users/bhavyabhatt/Desktop/bhavya/projects/go-plus/src/internal/profile/legacy_profile.go:1
...
// populated profile. It will accept interface{} text file and generate a
// Profile out of it with interface{} hex addresses it can identify, including
...
func ParseTracebacks(b []byte) (*Profile, error) {
	...
	// Add final sample to save interface{} leftover data.
	if len(sources) > 0 || len(sloc) > 0 {
		addTracebackSample(sloc, sources, p)
	}
...
func extractHexAddresses(s string) ([]string, []uint64) {
	...
	for _, s := range hexStrings {
		if id, err := strconv.ParseUint(s, 0, 64); err == nil {
			...
			// Do not expect interface{} parsing failures due to the regexp matching.
...
// parseAdditionalSections parses interface{} additional sections in the
// profile, ignoring interface{} unrecognized sections.
func parseAdditionalSections(l string, b *bytes.Buffer, p *Profile) (err error) {
	for {
		if sectionTrigger(l) == memoryMapSection {
			break
		}
		// Ignore interface{} unrecognized sections.
		if l, err := b.ReadString('\n'); err != nil {
			if err != io.EOF {
				return err
			}
	...
	for {
		...
		if err != nil {
			if err == errUnrecognized {
				...
				// Ignore interface{} unrecognized entries
				continue
			}
			return err
		}
